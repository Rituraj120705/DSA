...
Difficulty: MediumAccuracy: 43.65%Submissions: 174K+Points: 4Average Time: 30m
...
class Solution {
  public:
    void allpts(Node* root, vector<int>& ans, vector<vector<int>>& result){
        if(root==NULL){
            return;
        }
        ...
        if(root->left == NULL && root->right == NULL){
            ...
            allpts(root->left, ans, result);
            allpts(root->right, ans, result);
        }
