...
class Solution {
  public:
    bool ispossible(vector<int>&stalls,int mid,int k){          //k is how many cows are there 
        int n = stalls.size();
        int c =1;                                   //c is cow 
        int lastpo=stalls[0];                       //lastpo means lastposition 
        for(int i = 1; i<n; i++){
            if((stalls[i]-lastpo)>=mid){    //mid is a minimum allowed distance;
                c++;
                lastpo=stalls[i];
            }
            if(c==k)return true;
